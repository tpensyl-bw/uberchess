...
//
//*** Version .52b *** November 18, 2005 ***
//
//-housten, we have min/max!
//-fixed min max so it works for all depths now
//-added limited quiescence searching, still having problems
// with unlimited, goes into infinite loop.
//-uses alpha-beta, not confirmed workingyet, seems to increase speed tho.
//-IS NOT WORKING PROPERLY, MAKES NO EFFORT TO CAPTURE
//
//*** Things to add: ***
//
//-(easy) highlight last move made
//-AI (too slow, i need to speed it up)
...
///need to implement:
		winValue = pieceValue[Chess.KING] - (2 * pieceValue[Chess.KNIGHT] + 2 * pieceValue[Chess.BISHOP] + 2 * pieceValue[Chess.ROOK] + 9 * pieceValue[Chess.QUEEN]);
	}
...
String debug = "";
		...
		double bestScore = getScore(child, ChessAI.DEPTH, Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY);
		
		for(int i = 0; i < Array.getLength(legalMoves); i++) {
			child = board.branch(legalMoves[i]);
			double score = getScore(child, ChessAI.DEPTH, bestScore, Double.NEGATIVE_INFINITY);
			if(score > bestScore) {
				bestScore = score;
...
Chess.testString = debug + Double.toString(bestScore);
	...
	double getScore(Node board, int d, double bestScore, double worstScore) {
		Move[] legalMoves = board.getAllMoves();
// || Array.getLength(legalMoves) == 0
		...
		for(int i = 0; i < Array.getLength(legalMoves); i++) {
			child = board.branch(legalMoves[i]);
			double score = -getScore(child, d - 1, -bestScore, -worstScore);
			if(score > worstScore) {
				return worstScore;
			}
			if(score > bestScore) {
...
//
	...
	public Move[] getPieceMoves(int x, int y) {
		...
		if(movee == Chess.KING * currentPlayer) {
			...
			movesTemp.add(get1DMoves(x, y, -1, 1));
			movesTemp.add(get1DMoves(x, y, -1, -1));
		} else if(movee == Chess.PAWN * currentPlayer) {
