...
//
//*** Version .10i (AKA ChessOnCrack) *** November 17, 2005 ***
//
//-computer now makes random moves... the only problem is it doesn't know when to stop. ø.Ø
//-i also have no clue why black is so "overpowered" hehe
//
//*** Things to add: ***
//
//-turn system
...
public class Chess extends Applet implements Runnable {
	...
	public void run() {
		...
		while (mainLoop == thisThread) {			
			if(board.currentPlayer == Chess.BLACK) {
...
class Chessboard{
	...
	public boolean isLegal(Move tryMove) {
		...
		//(actually its inefficient even for this purpose, but at least it works.
		//instead, used when a human clicks a space.
